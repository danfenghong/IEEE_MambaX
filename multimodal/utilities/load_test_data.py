...
def load_h5py_with_hp(file_path):
    ...
    lms1 = data["lms"][...]  # W H C N
    lms = np.array(lms1, dtype=np.float32) / 2047.
    lms = torch.from_numpy(lms)
    return ms, ms_hp, pan, pan_hp, lms


def load_h5py1(file_path):
    data = h5py.File(file_path)
    ms = data["ms"][...]  # W H C N
    ms = np.array(ms, dtype=np.float32) / 1023.
    ms = torch.from_numpy(ms)
    lms = data["lms"][...]  # W H C N
    lms = np.array(lms, dtype=np.float32) / 1023.
    lms = torch.from_numpy(lms)
    pan = data["pan"][...]  # W H C N
    pan = np.array(pan, dtype=np.float32) / 1023.
    pan = torch.from_numpy(pan)

    gt = data["gt"][...]  # W H C N
    gt = np.array(gt, dtype=np.float32) / 1023.
    gt = torch.from_numpy(gt)
    return ms, lms, pan, gt


def load_h5py_with_hp1(file_path):
    data = h5py.File(file_path)
    ms1 = data["ms"][...]
    ms = np.array(ms1, dtype=np.float32) / 1023.
    ms = torch.from_numpy(ms)
    ms_hp = np.array(ms1.transpose(0, 2, 3, 1), dtype=np.float32) / 1023.  # NxHxWxC
    ms_hp = get_edge_h5py(ms_hp)
    ms_hp = torch.from_numpy(ms_hp).permute(0, 3, 1, 2)
    pan1 = data["pan"][...]  # W H C N
    pan = np.array(pan1, dtype=np.float32) / 1023.
    pan = torch.from_numpy(pan)
    pan_hp = np.array(pan1.transpose(0, 2, 3, 1), dtype=np.float32) / 1023.  # NxHxWx1
    pan_hp = np.squeeze(pan_hp, axis=3)  # NxHxW
    pan_hp = get_edge_h5py(pan_hp)  # NxHxW
    pan_hp = np.expand_dims(pan_hp, axis=3)  # NxHxWx1
    pan_hp = torch.from_numpy(pan_hp).permute(0, 3, 1, 2)
    lms1 = data["lms"][...]  # W H C N
    lms = np.array(lms1, dtype=np.float32) / 1023.
    lms = torch.from_numpy(lms)
    return ms, ms_hp, pan, pan_hp, lms
